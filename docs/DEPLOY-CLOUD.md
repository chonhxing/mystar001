...
| 本地存档写不进去也无所谓 | 容器日志可能出现「本地存档写入失败」——**数据不会丢**（MySQL 那份是独立的），但说明存档目录不可写。容器本来就没有持久化磁盘，DATA_DIR 已指到 `/tmp/wotui-data` |
| 容器**真的起来了** | 控制台「运行日志」里应该看到我们的启动横幅：`我推的星运 · 后端已启动` + `存储: MySQL 快照（单实例）`。⚠️ 若看到 `Error: listen EACCES ... port: 80`，就是镜像又在非 root 下监听 80（Dockerfile 末尾别加 `USER node`） |
| 部署成功的**唯一判据**：`/api/health` 返回 JSON | 浏览器打开 `https://<默认域名>/api/health`，应返回 `{"ok":true,...}`。⚠️ 如果返回的是 **Express 的 404 页**（`Cannot GET /api/health`），说明新版本的 pod 没起来、域名还由上一个版本（模板示例应用）应答 —— 控制台"线上版本"那一栏会显示新版本号，**别被它骗了**，去看运行日志 |
