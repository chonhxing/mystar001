...
## 4.5 ⚠️ 两个"发布动作"完全不是一回事（我们被坑过一次）

云托管里能触发发布的地方有两个，**发布的东西不一样**：

| 动作 | 发布的是 | 后果 |
| --- | --- | --- |
| **推代码到 GitHub**（或在「流水线」里手动构建） | **我们的镜像**（走仓库里的 Dockerfile） | ✅ 这才是我们要的 |
| 在「服务设置」里点**保存** | ⚠️ **官方模板的示例应用**（页面写着"基于模板开发/官方模板"） | ❌ 会把我们的服务顶掉，所有接口变成 `Cannot GET /api/health` 之类的 404 |

我们就是这么踩进去的：改完环境变量顺手点了保存，结果线上从"我们的服务"
变成了模板的 Express 示例应用（日志里是 `wxcloudrun-express@1.0.0 start /app`、
`Executing (default): ... 'nodejs_demo'`），真机上表现为"连得上服务器但账号连不上"。

**正确顺序**：先把环境变量、副本数这些在「服务设置」里改好保存 →
**然后推一次代码**（或在「流水线」里点构建），用我们的镜像重新发布一次。
环境变量是运行时注入的，重新发布后自然生效；不需要为了让它生效去动服务设置。

**怎么一眼看出当前跑的是谁**：去「运行日志」看启动横幅。
我们的服务会打印 `我推的星运 · 后端已启动` + `存储: MySQL 快照（单实例）`；
模板应用则是 `> wxcloudrun-express@1.0.0 start /app`。

---

## 4.6 真微信登录：那个 `微信登录: 开发模式` 是怎么来的

启动横幅里的登录方式由 `WX_APPID` + `WX_SECRET` 决定：

- 两个都配了 → 真微信登录（`wx.login` 换 openid，跨设备认得同一个人）
- 少任何一个 → **开发模式**（按设备 id 认人），日志和 `/api/health` 都会如实报出来

⚠️ 这里的 `WX_SECRET` 是**小程序的 AppSecret**（32 位十六进制），
不是 AppID（`wx` 开头那种，两个都要填）。改完记得**重新发布一次我们的镜像**
（见上一节），否则环境变量不生效。
...
### ⚠️ 「公网访问」和「公网出口」是两个开关，别搞混（我们在这上面卡过）

服务设置里有两栏，名字很像，方向相反：

| 开关 | 方向 | 不开的后果 |
| --- | --- | --- |
| **公网访问** | **入**：外面能不能访问到你的服务 | 外部访问不了（但云调用/内网不受影响） |
| **公网出口** | **出**：容器能不能主动访问外网 | ⚠️ **AI 和微信登录全废**：连不上 `api.deepseek.com`、连不上 `api.weixin.qq.com` |

**症状长这样**（真机上表现为"账号登录不上"、"AI 永远是本机模板文案"）：

```
POST /api/account/relogin 401 4ms      ← 快到 1 秒内就失败，说明压根没出网
{"ok":false,"error":"WX_UNREACHABLE"}  ← 服务端给的原因
/api/health 里 stats.calls 一直是 0    ← 一次 AI 都没成功过
```
...
但既然 **AI 已经能走云调用**（见第 5 节），**建议直接把「公网访问」关掉**，
只留云调用 —— 少一个暴露面，也顺带省掉域名白名单的维护。
（注意：关的是"**访问**"，"出口"必须保持开着，否则 AI 和登录都会挂。）
